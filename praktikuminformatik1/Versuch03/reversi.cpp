...
*/
bool aufSpielfeld(const int posY, const int posX)
{
    // Hier erfolgt jetzt Ihre Implementierung ...
	if(posY<=GROESSE_Y-1 && 0<=posY)
	{
		if(posX<=GROESSE_X-1 && 0<=posX)
		{
		return true;
		}
		return false;
...
bool zugGueltig(const int spielfeld[GROESSE_Y][GROESSE_X], const int aktuellerSpieler, const int posX, const int posY)
{
    ...
    for (int j = -1; j <= 1; j++)
    {
        for (int i = -1; i <= 1; i++)
        {
        		...
        		{
        			 int k = 2;
        			 	 	 	 	while(aufSpielfeld(posY+(i*k),posX+(j*k)))
        				            {
        				        	if(spielfeld[posY+i*k][posX+j*k]==0){break;}
        				        	else if(spielfeld[posY+i*k][posX+j*k]==aktuellerSpieler){return true;}
        				            k++;
        				            }
        		}
        	...
        	if(spielfeld[posY+i][posX+j] == gegner)
        	        		{
        	        			 int k = 2;
        	        				            ...
        	        				            {
        	        				        	if(spielfeld[posY+i*k][posX+j*k]==0){break;}
        	        				        	else if(spielfeld[posY+i*k][posX+j*k]==aktuellerSpieler)
        	        				        	{
        	        				        		for(int l = k-1; l > 0; l--)
        	        				        								{
        	        				        									// Go path back again and set all fields to player
        	        				        									spielfeld[posY + l * i][posX + l * j] = aktuellerSpieler;
        	        				        								}
        	        				        								break;
...
void spielen(const int spielerTyp[2])
{
    ...
    while(moeglicheZuege(spielfeld, aktuellerSpieler) || moeglicheZuege(spielfeld, 3 - aktuellerSpieler))
    {
    	 ...
    	 else if(spielerTyp[aktuellerSpieler-1]==COMPUTER){computerZug(spielfeld, aktuellerSpieler);}
    	 zeigeSpielfeld(spielfeld);
